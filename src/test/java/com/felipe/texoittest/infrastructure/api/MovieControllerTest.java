...
public class MovieControllerTest {
    ...
    public void testFindAllMovies() throws Exception {
        mockMvc.perform(get("/movies/ranking-worst-awards"))
                ...
                .andExpect(jsonPath("$.max[0].followingWin").value(2037));
    }

    @Test
    public void testFindAllMoviesWhoWinnerMoreForAwards() throws Exception {
        mockMvc.perform(get("/movies/ranking-worst-awards"))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.min[0].producer").value("Joel Silver"))
        ...
        MovieModel movieModel = movieRepository.save(new MovieModel("Filme 1", 2067, "Studio de teste", true));
        ProducerModel producerModel = producerRepository.findById(152).orElse(null);
        ...
        mockMvc.perform(get("/movies/ranking-worst-awards"))
                ...
                .andExpect(jsonPath("$.min[1].followingWin").value(2003))
                .andExpect(jsonPath("$.max[0].producer").value("Matthew Vaughn"))
                .andExpect(jsonPath("$.max[0].interval").value(30))
                .andExpect(jsonPath("$.max[0].previousWin").value(2037))
                .andExpect(jsonPath("$.max[0].followingWin").value(2067));
    }
